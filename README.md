...
- **Reporting** — anyone can download a CSV snapshot of the year's expense log from the Dashboard, which also logs that the download happened. From the Approvals page, a signed-in board member can also have the full report (every expense, every status) emailed to themselves. See `src/lib/reports.ts` and `supabase/functions/send-approvals-report/`.
...
| **Report email** (full approvals report emailed to the requester) | On-demand — fires whenever a signed-in user clicks "Send report to email" on the Approvals page | `supabase/functions/send-approvals-report/`, logged to `report_downloads` with `channel = 'email'` |
...
4. Deploy the review edge function and give it an Anthropic API key (it runs server-side, so the key never reaches the browser):
   ...
   ```
5. Deploy the report-email edge function and give it a [Resend](https://resend.com) API key (free tier). Without a verified sending domain in Resend, `REPORT_FROM_EMAIL` must be `onboarding@resend.dev`, which can only deliver to the email address on your Resend account — verify a domain in Resend to send to any club member:
   ```
   supabase functions deploy send-approvals-report
...
- `src/pages/` — route-level screens (`Dashboard`, `SubmitExpense`, `Approvals`, `Budgets`, `Login`, `Signup`)
...
- `supabase/functions/send-approvals-report/` — the on-demand report-email step (see below)
...
- **Category budgets are editable from `/budgets`, also behind auth.** The Dashboard reads and displays budgets to anyone, but setting them (`src/pages/Budgets.tsx`) is wrapped in the same `ProtectedRoute` as Approvals. Writes to `category_budgets` require an authenticated session at the RLS level too (`supabase/migrations/20260814010000_restrict_category_budget_writes.sql`) — reads stay open to anon so the public Dashboard keeps working without login.
- **Report email is a fixed workflow, not agentic** — same steps every time: verify the caller is signed in, pull every expense (all statuses, no year filter), build a CSV, email it via Resend, log it. The recipient is always the signed-in user's own email, read from their verified session server-side — the request body is never trusted for who to email, so there's no way to use the button to send the report to someone else's inbox.
